...
function($, Backbone, _, _s, TaskStatus){
    ...
    var processTaskStatus = function(taskData){
        ...
        if (taskData.status == 'resolved'){
            taskData.data.stages.links = {
                status: {code: 'running'}
            };
            updateTracker(taskData);

            // Get file info.
            var url_template = "{{ url_for('sasi_file.getFileData', file_id=-999) }}";
            var url = url_template.replace('-999', taskData.data.result.file_id);
            $.ajax({
                url: url,
                type: 'GET',
                success: function(file_data){
                    var file_name = file_data.filename;
                    var file_link = file_data.url;
                    var results_page_link = "{{url_for('sasi_result.list_results')}}";
                    var msg = _s.sprintf(
                    ...
                    msg += _s.sprintf(
                        ...
                        '<a href="%s">results page</a>.</p>',
                        results_page_link);
                    taskData.data.stages.links.status.code = 'resolved';
                    taskData.data.success_msg = msg;
                    taskData.data.progress = 100.0;
